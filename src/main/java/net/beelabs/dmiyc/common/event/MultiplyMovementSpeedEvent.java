...
public interface MultiplyMovementSpeedEvent {
    ...
    static float capMovementMultiplier(float multiplier) {
        return Math.min(MAXIMUM_MULTIPLIER, multiplier);
    }
    ...
    static float getMovementMultiplier(LivingEntity entity) {
        ...
        if (entity.isUsingItem()) {
            if (entity.getActiveItem().getItem() instanceof BowItem) {
                multiplier *= 0.6F;
            } else {
                multiplier *= 0.2;
            }
       ...
       // multiplier = Math.min(ModifyMovementSpeedEvent.MAXIMUM_MULTIPLIER, multiplier);
